...
class LlavaMetaModel:
    ...
    def __init__(self, config):
        ...
        if hasattr(config, "mm_pool_size"):
            self.token_pool = nn.Parameter(torch.randn(config.mm_pool_size, self.config.hidden_size))
            self.pool_weight = nn.Linear(self.config.hidden_size, self.config.hidden_size)
    ...
    def initialize_vision_modules(self, model_args, fsdp=None):
        ...
        self.token_pool = nn.Parameter(torch.randn(model_args.mm_pool_size, self.config.hidden_size))
        self.pool_weight = nn.Linear(self.config.hidden_size, self.config.hidden_size)
...
class LlavaMetaForCausalLM(ABC):
    ...
    def encode_images(self, images, grid_thw=None, victor_stage=False):
        image_features = self.get_model().get_vision_tower()(images, grid_thw)
        image_features = [self.get_model().mm_projector(feat) for feat in image_features]
        image_features = [torch.nn.functional.normalize(feat, p=2, dim=-1) for feat in image_features]
        bs = len(image_features)
        
        select_patch = []
        for i in range(bs):
            feat_num = image_features[i].shape[0]
            num_unmask = feat_num // 4 
            unmask_indices = torch.randperm(feat_num)[num_unmask:]
            cur_select_patch = torch.zeros(feat_num, dtype=torch.bool, device=image_features[i].device)
            cur_select_patch[unmask_indices] = True
            select_patch.append(cur_select_patch)
    ...
    def prepare_inputs_labels_for_multimodal(self, input_ids, position_ids, attention_mask, past_key_values, labels, images, modalities=["image"], image_sizes=None, grid_thw=None, victor_stage=False):
        ...
        if type(images) is list or images.ndim == 5:
            ...
            encoded_image_features, select_patch = self.encode_images(concat_images, grid_thw, victor_stage)

            # This is a list, each element is [num_images, patch * patch, dim]
            # rank_print(f"Concat images : {concat_images.shape}")
            encoded_image_features = torch.split(encoded_image_features, split_sizes) if type(encoded_image_features) is torch.Tensor else encoded_image_features
            image_features = []
            ...
            image_features, select_patch = self.encode_images(images, grid_thw, victor_stage)
        if type(image_features) is list:
            ori_image_features = [x.clone().float().detach() for x in image_features]
        else:
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            if num_images == 0:
                ...
                new_img_labels.append(torch.zeros([cur_input_embeds.shape[0], cur_input_embeds.shape[-1]], dtype=cur_input_embeds.dtype, device=self.device))
            ...
            for i in range(num_images + 1):
                ...
                cur_new_img_labels.append(torch.zeros([cur_labels_noim[i].shape[0], cur_input_embeds_no_im[i].shape[-1]],dtype=cur_input_embeds_no_im[i].dtype, device=self.device))
                ...
                if i < num_images:
                    try:
                        ...
                        cur_ori_image_features = ori_image_features[cur_image_idx-1]
                    cur_image_idx += 1
        ...
        for i, (cur_new_embed, cur_new_labels) in enumerate(zip(new_input_embeds, new_labels)):
            ...
            if getattr(self.config, "tokenizer_padding_side", "right") == "left":
                ...
                new_img_labels_padded.append(torch.cat((
                    cur_new_img_labels,
                    torch.zeros((max_len - cur_len, ori_image_features[i].shape[-1]), dtype=ori_image_features[i].dtype, device=cur_new_embed.device)
                ), dim=0))
                if cur_len > 0:
                    new_labels_padded[i, :cur_len] = cur_new_labels
                    attention_mask[i, :cur_len] = True
        ...
        if victor_stage:
            for i in range(new_img_pos.shape[0]):
                ...
                dist_matrix = torch.cdist(new_input_embeds[i][new_select_patch[i]], self.model.token_pool)

                dist_matrix_cpu = dist_matrix.detach().float().cpu().numpy()
                
                row_indices, col_indices = linear_sum_assignment(dist_matrix_cpu)
                
                col_indices_tensor = torch.tensor(col_indices, device=new_input_embeds.device)
                
                matched = self.model.token_pool[col_indices_tensor]
