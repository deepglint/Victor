...
class LLaVATrainer(Trainer):
    ...
    def log(self, logs: Dict[str, float], start_time=None) -> None:
        ...
        if hasattr(self, '_current_log_dict') and self._current_log_dict:
            new_logs = self._current_log_dict.copy()
            new_logs.update(logs)
            logs = new_logs

        now = datetime.now()
        if len(self.state.log_history) > 0 and 'time' in self.state.log_history[-1]:
            last_time = self.state.log_history[-1]['time']
            if isinstance(last_time, str):
                last_time = datetime.fromisoformat(last_time)
            duration = now - last_time
            log_keys = list(logs.keys()) + ['step_time']
            logs['step_time'] = f'{duration.total_seconds():.2f}s'
            logs = dict([(key, logs[key]) for key in log_keys])
        now = datetime.now().isoformat()
        output = {**logs, **{"step": self.state.global_step, 'time': str(now)}}
    ...
    def create_optimizer(self):
        ...
        if self.optimizer is None:
            ...
            if self.args.visual_token_pool_lr is not None:
                lr_mapper["token_pool"] = self.args.visual_token_pool_lr
                lr_mapper["pool_weight"] = self.args.visual_token_pool_lr
            if len(lr_mapper) > 0:
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        if type(outputs) is tuple and type(outputs[-1]) is dict:
            loss_dict = outputs[-1]
            if not hasattr(self, '_current_log_dict'):
                self._current_log_dict = {}
            self._current_log_dict.update(loss_dict)
        elif hasattr(outputs, 'log_dict') and outputs.log_dict:
            if not hasattr(self, '_current_log_dict'):
                self._current_log_dict = {}
            self._current_log_dict.update(outputs.log_dict)
