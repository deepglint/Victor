...
class ModelArguments:
    ...
    mm_pool_size: Optional[int]  = field(default=2048)
...
class DataArguments:
    ...
    victor_stage: bool = field(default=False, metadata={"help": "Whether to use CROC dataset."})
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    visual_token_pool_lr: Optional[float] = None
...
class LazySupervisedDataset(Dataset):
    ...
    def process_image(self, image_file, overwrite_image_aspect_ratio=None):
        ...
        if image_aspect_ratio == "highres":
            ...
            image_dict = process_anyres_image(image, self.data_args.image_processor, self.data_args.image_grid_pinpoints)
            image = image_dict['image_patchs'].squeeze(0)
            grid_thw = image_dict['grid_thw']
            ...
            if getattr(processor, "image_processor", None) is not None and hasattr(processor.image_processor, "image_patchs"):
                image_dict = processor.image_processor(image, return_tensors="pt")["pixel_values"][0]
                image = image_dict['image_patchs'].squeeze(0)
                grid_thw = image_dict['grid_thw']
            else:
                image = processor.preprocess(image, return_tensors="pt")["pixel_values"][0]
        else:
            # assert 1==2, f'processor:{processor}, class:{processor.__class__}, video_processor:{processor.image_processor}'
            ...
            if getattr(processor, "image_processor", None) is not None:
                ...
                grid_thw = torch.tensor([[1, 560 // 14, 560 // 14]])
            else:
                image = processor.preprocess(image, return_tensors="pt")["pixel_values"][0]
            
            # assert 1==2, f'image.shape:{image.shape}, image_size:{image_size}, image_aspect_ratio:{image_aspect_ratio}'
    ...
    def _get_item(self, i) -> Dict[str, torch.Tensor]:
        ...
        if "image" in self.list_data_dict[i]:
            ...
            # crop_size = {'width':560, 'height':560} # self.data_args.image_processor.crop_size #FIXME

            # data_dict["image"] = [
            #     (torch.zeros(1600, 1176).float(), (crop_size["width"], crop_size["height"]), "text"),
            # ]
            rank0_print(f"Warning: image does not exist in the data, but the model is multimodal. Using a dummy image.")
...
class DataCollatorForSupervisedDataset(object):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        if "image" in instances[0]:
            ...
            batch['grid_thw'] = [im[3] if len(im) > 3 else None for im_list in images for im in im_list]
            batch['victor_stage'] = True if self.victor_stage else False
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer, data_args) -> Dict:
    ...
    data_collator = DataCollatorForSupervisedDataset(tokenizer=tokenizer, victor_stage=data_args.victor_stage)
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        ...
        model.config.mm_pool_size = model_args.mm_pool_size
